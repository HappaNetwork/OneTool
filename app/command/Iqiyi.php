...
class Iqiyi extends Command
{
    ...
    protected function execute(Input $input, Output $output)
    {
        ...
        $vip_expired_userIds = [];
        $jobs = Jobs::where([['type', '=', 'iqiyi'], ['state', '=', 1], ['nextExecute', '<=', time()]])
            ->limit((int)$interval)
            ->select();
        foreach ($jobs as $job) {
            ...
            $do = new IqiyiAPI($account_info['P00001'], $account_info['P00003'], $job_config);
            $execute = $do->{$job['do']}();
            if ($do->cookiezt) {
                $account = Accounts::where('type', '=', 'iqiyi')->where('user_id', '=', $job['user_id'])->find();
                $user = Users::where('uid', '=', $account['uid'])->find();
                $this->accountInvalid('iqiyi', $user, $job['user_id']); // 账号失效处理
                break;
            } else {
                TaskLogs::operateExecuteLog('iqiyi', $job['user_id'], $job['do'], $execute['message']); // 写入运行日志
            }
    ...
    protected function accountInvalid($type, $user, $user_id)
    {
        ...
        if (config('sys.mail_invalid') == 1) {
            $msg = get_mail_tempale(3, $user, '爱奇艺');
