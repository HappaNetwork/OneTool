...
class Bilibili extends Command
{
    ...
    protected function execute(Input $input, Output $output)
    {
        ...
        $vip_expired_userIds = [];
        $jobs = Jobs::where([[['do', '<>', 'globalroom']], ['type', '=', 'bilibili'], ['state', '=', 1], ['nextExecute', '<=', time()]])
            ->limit((int)$interval)
            ->select();
        foreach ($jobs as $job) {
            ...
            $do = new BiliHelper($account_info['mid'], $account_info['mid_md5'], $account_info['token'], $account_info['csrf'], $account_info['access_key'], $job_config);
            $execute = $do->{$job['do']}();
            if ($do->cookiezt) {
                $account = Accounts::where('type', '=', 'bilibili')->where('user_id', '=', $job['user_id'])->find();
                $user = Users::where('uid', '=', $account['uid'])->find();
                $this->accountInvalid('bilibili', $user, $job['user_id']); // 账号失效处理
                break;
            } else {
                TaskLogs::operateExecuteLog('bilibili', $job['user_id'], $job['do'], $execute['message']); // 写入运行日志
            }
    ...
    protected function accountInvalid($type, $user, $user_id)
    {
        ...
        if (config('sys.mail_invalid') == 1) {
            $msg = get_mail_tempale(3, $user, '哔哩哔哩');
