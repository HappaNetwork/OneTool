...
class Mihoyo extends Command
{
    ...
    protected function execute(Input $input, Output $output)
    {
        ...
        $vip_expired_userIds = [];
        $jobs = Jobs::where([['type', '=', 'mihoyo'], ['state', '=', 1], ['nextExecute', '<=', time()]])
            ->limit((int)$interval)
            ->select();
        foreach ($jobs as $job) {
            ...
            $do = new MihoyoAPI($account_info);
            $execute = $do->{$job['do']}();
            if ($do->cookiezt) {
                $account = Accounts::where('type', '=', 'mihoyo')->where('user_id', '=', $job['user_id'])->find();
                $user = Users::where('uid', '=', $account['uid'])->find();
                $this->accountInvalid('mihoyo', $user, $job['user_id']); // 账号失效处理
                break;
            } else {
                TaskLogs::operateExecuteLog('mihoyo', $job['user_id'], $job['do'], $execute['message']); // 写入运行日志
            }
    ...
    protected function accountInvalid($type, $user, $user_id)
    {
        ...
        if (config('sys.mail_invalid') == 1) {
            $msg = get_mail_tempale(3, $user, '米游社');
