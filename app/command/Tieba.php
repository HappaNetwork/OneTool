...
class Tieba extends Command
{
    ...
    protected function execute(Input $input, Output $output)
    {
        ...
        $vip_expired_userIds = [];
        $jobs = Jobs::where([['type', '=', 'tieba'], ['state', '=', 1], ['nextExecute', '<=', time()]])
            ->limit((int)$interval)
            ->select();
        foreach ($jobs as $job) {
            ...
            if ($task['vip'] == 1 && strtotime($user['vip_start'] ?? '') < time()) {  // 判断会员功能、用户会员是否过期
            ...
            $do = new TiebaHelper($account_info);
            $execute = $do->{$job['do']}();
            if ($do->cookiezt) {
                $account = Accounts::where('type', '=', 'tieba')->where('user_id', '=', $job['user_id'])->find();
                $user = Users::where('uid', '=', $account['uid'])->find();
                $this->accountInvalid('tieba', $user, $job['user_id']); // 账号失效处理
                break;
            } else {
                foreach ($do->msg as $message) {
                    TaskLogs::operateExecuteLog('tieba', $job['user_id'], $job['do'], $message); // 写入运行日志
                }
            }
    ...
    protected function accountInvalid($type, $user, $user_id)
    {
        ...
        if (config('sys.mail_invalid') == 1) {
            $msg = get_mail_tempale(3, $user, '百度贴吧');
